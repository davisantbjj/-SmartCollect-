...
public class TitlesController : ControllerBase
{
    ...
    private Guid ResolveTenantIdForWrite(Guid? tenantId)
        => TenantContextResolver.ResolveTenantOrThrow(User, tenantId);
    ...
    [Authorize(Roles = "Admin,Worker,Master")]
    public async Task<IActionResult> UpdateStatus(Guid id, [FromBody] UpdateTitleStatusRequest request, [FromQuery] Guid? tenantId = null)
    {
        try
        {
            var result = await _titleService.UpdateStatusAsync(ResolveTenantIdForWrite(tenantId), id, request.Status);
            return result is null ? NotFound() : Ok(result);
        }
        catch (InvalidOperationException ex)
        {
    ...
    [HttpPost]
    [Authorize(Roles = "Admin,Worker,Master")]
    public async Task<IActionResult> Create([FromBody] CreateTitleRequest request, [FromQuery] Guid? tenantId = null)
    {
        try
        {
            return Ok(await _titleService.CreateAsync(ResolveTenantIdForWrite(tenantId), request));
        }
        catch (InvalidOperationException ex)
        {
            return BadRequest(new { message = ex.Message });
    ...
    [Authorize(Roles = "Admin,Worker,Master")]
    public async Task<IActionResult> SendCollection(Guid id, [FromBody] SendCollectionRequest? request = null, [FromQuery] Guid? tenantId = null)
    {
        try
        {
